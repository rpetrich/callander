...
static inline intptr_t apply_base_address(const struct binary_info *info, intptr_t addr_or_offset)
{
...
__attribute__((warn_unused_result))
extern int load_dynamic_symbols(int fd, const struct binary_info *info, struct symbol_info *out_symbols);
// parse_dynamic_symbols will parse dynamic symbols into a symbol struct
__attribute__((warn_unused_result))
extern int parse_dynamic_symbols(const struct binary_info *info, void *mapped_address, struct symbol_info *out_symbols);
...
__attribute__((warn_unused_result))
extern void *find_next_symbol(const struct binary_info *info, const struct symbol_info *symbols, const char *symbol_name, const ElfW(Sym) **symbol);
// find_symbol_by_address looks up a symbol by internal address
__attribute__((warn_unused_result))
extern void *find_symbol_by_address(const struct binary_info *info, const struct symbol_info *symbols, const void *addr, const ElfW(Sym) **out_symbol);
...
__attribute__((always_inline))
static inline struct symbol_version_info symbol_version_for_index(const struct symbol_info *symbols, ElfW(Half) index)
{
