...
test_program () {
	...
	if [ -g "$prog" -o -u "$prog" ]; then
		rm "$SCRIPT_DIR/callander_fixtures/$filename".txt
	else
		if [ "/sbin/$filename" = "$prog" -a -e "/bin/$filename" ]; then
			# skipping since duplicate binary in /sbin
			return
		else
			"$SCRIPT_DIR/callander" --skip-running --show-permitted --block-syscall execve --block-syscall execveat --ignore-dlopen $flags-- "$prog" 2> "$SCRIPT_DIR"/callander_fixtures/"$filename"_new.txt
			if [ "$?" != 0 ]; then
				echo "\"callander --skip-running --show-permitted --block-syscall execve --block-syscall execveat --ignore-dlopen $flags-- $prog\" failed"
			fi
			if [ -e "$SCRIPT_DIR"/callander_fixtures/"$filename".txt ]; then
				diff=$(unbuffer git --no-pager diff --no-index -- "$SCRIPT_DIR"/callander_fixtures/"$filename".txt "$SCRIPT_DIR"/callander_fixtures/"$filename"_new.txt)
				if [ "$?" != 0 ]; then
					echo "\"callander --skip-running --show-permitted --block-syscall execve --block-syscall execveat --ignore-dlopen $flags-- $prog\" changed"
					echo "$diff"
				fi
			fi
			mv "$SCRIPT_DIR/callander_fixtures/$filename"{_new,}.txt
		fi
	fi
}
...
if [ "$1" != "" ]; then
	...
	run_and_diff du -h "$0"
	run_and_diff env -u _ -u LD_PRELOAD
	run_and_diff false
	...
	# exit 0
	parallel "$0" -- /bin/* /sbin/*
	echo $(grep -R -e 'permitted syscalls' -- "$SCRIPT_DIR/callander_fixtures/" | wc -l)/$(ls "$SCRIPT_DIR/callander_fixtures/" | wc -l) binaries successfully processed
	echo failing tests:
	grep -R -e 'callander: ' -- "$SCRIPT_DIR/callander_fixtures/" | grep -v 'permitted' | cut -d ':' -f 1 | uniq
fi
