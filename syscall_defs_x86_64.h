SYSCALL_DEF(read, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(write, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(open, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_MODEFLAGS_OF(0))
SYSCALL_DEF(close, 0, SYSCALL_ARG_IS_FD)
SYSCALL_DEF(stat, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(fstat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(lstat, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(poll, SYSCALL_IS_RESTARTABLE, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(0), SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(lseek, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_OFFSET, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(mmap, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0), SYSCALL_ARG_IS_PROT, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_OFFSET)
SYSCALL_DEF(mprotect, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0), SYSCALL_ARG_IS_PROT)
SYSCALL_DEF(munmap, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0))
SYSCALL_DEF(brk, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(rt_sigaction, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(rt_sigprocmask, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(rt_sigreturn, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(ioctl, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(pread64, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1), SYSCALL_ARG_IS_OFFSET)
SYSCALL_DEF(pwrite64, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1), SYSCALL_ARG_IS_OFFSET)
SYSCALL_DEF(readv, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1))
SYSCALL_DEF(writev, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1))
SYSCALL_DEF(access, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(pipe, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(select, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(sched_yield, 0)
SYSCALL_DEF(mremap, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0), SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(msync, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(mincore, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(2), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(madvise, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1), SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(shmget, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(shmat, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(shmctl, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(dup, 0, SYSCALL_ARG_IS_FD)
SYSCALL_DEF(dup2, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_FD)
SYSCALL_DEF(pause, 0)
SYSCALL_DEF(nanosleep, SYSCALL_IS_RESTARTABLE, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(getitimer, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(alarm, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(setitimer, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(getpid, 0)
SYSCALL_DEF(sendfile, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(socket, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(connect, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(accept, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(sendto, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1), SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(4))
SYSCALL_DEF(recvfrom, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1), SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(sendmsg, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(recvmsg, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(shutdown, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(bind, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(listen, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(getsockname, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(getpeername, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(socketpair, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(setsockopt, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(3))
SYSCALL_DEF(getsockopt, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(clone, 0, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(fork, 0)
SYSCALL_DEF(vfork, 0)
SYSCALL_DEF(execve, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(exit, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(wait4, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(kill, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(uname, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(semget, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(semop, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1))
SYSCALL_DEF(semctl, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(shmdt, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(msgget, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(msgsnd, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(msgrcv, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1), SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(msgctl, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(fcntl, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(flock, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(fsync, 0, SYSCALL_ARG_IS_FD)
SYSCALL_DEF(fdatasync, 0, SYSCALL_ARG_IS_FD)
SYSCALL_DEF(truncate, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_OFFSET)
SYSCALL_DEF(ftruncate, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_OFFSET)
SYSCALL_DEF(getdents, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1))
SYSCALL_DEF(getcwd, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0))
SYSCALL_DEF(chdir, 0, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(fchdir, 0, SYSCALL_ARG_IS_FD)
SYSCALL_DEF(rename, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(mkdir, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(rmdir, 0, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(creat, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(link, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(unlink, 0, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(symlink, 0, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(readlink, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(chmod, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(fchmod, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(chown, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(fchown, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(lchown, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(umask, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(gettimeofday, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(getrlimit, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(getrusage, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(sysinfo, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(times, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(ptrace, 0, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(getuid, 0)
SYSCALL_DEF(syslog, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(getgid, 0)
SYSCALL_DEF(setuid, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(setgid, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(geteuid, 0)
SYSCALL_DEF(getegid, 0)
SYSCALL_DEF(setpgid, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(getppid, 0)
SYSCALL_DEF(getpgrp, 0)
SYSCALL_DEF(setsid, 0)
SYSCALL_DEF(setreuid, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(setregid, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(getgroups, 0, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(setgroups, 0, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(setresuid, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(getresuid, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(setresgid, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(getresgid, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(getpgid, 0, SYSCALL_ARG_IS_PID)
SYSCALL_DEF(setfsuid, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(setfsgid, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(getsid, 0, SYSCALL_ARG_IS_PID)
SYSCALL_DEF(capget, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(capset, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(rt_sigpending, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(rt_sigtimedwait, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0))
SYSCALL_DEF(rt_sigqueueinfo, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(rt_sigsuspend, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0))
SYSCALL_DEF(sigaltstack, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(utime, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(mknod, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(uselib, 0, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(personality, 0, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER))
SYSCALL_DEF(ustat, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(statfs, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(fstatfs, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(sysfs, 0, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(getpriority, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(setpriority, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(sched_setparam, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(sched_getparam, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(sched_setscheduler, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(sched_getscheduler, 0, SYSCALL_ARG_IS_PID)
SYSCALL_DEF(sched_get_priority_max, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(sched_get_priority_min, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(sched_rr_get_interval, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(mlock, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0))
SYSCALL_DEF(munlock, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0))
SYSCALL_DEF(mlockall, 0, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(munlockall, 0)
SYSCALL_DEF(vhangup, 0)
SYSCALL_DEF(modify_ldt, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(pivot_root, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(_sysctl, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(prctl, 0, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(arch_prctl, 0, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(adjtimex, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(setrlimit, 0, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(chroot, 0, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(sync, 0)
SYSCALL_DEF(acct, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(settimeofday, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(mount, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(umount2, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(swapon, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(swapoff, 0, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(reboot, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(sethostname, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0))
SYSCALL_DEF(setdomainname, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0))
SYSCALL_DEF(iopl, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(ioperm, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(create_module, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0))
SYSCALL_DEF(init_module, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(delete_module, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(get_kernel_syms, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(query_module, 0, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(2), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(quotactl, 0, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(nfsservctl, 0, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(getpmsg, 0)
SYSCALL_DEF(putpmsg, 0)
SYSCALL_DEF(afs_syscall, 0)
SYSCALL_DEF(tuxcall, 0)
SYSCALL_DEF(security, 0)
SYSCALL_DEF(gettid, 0)
SYSCALL_DEF(readahead, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_OFFSET, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(setxattr, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(2), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(lsetxattr, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(2), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(fsetxattr, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(2), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(getxattr, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(2))
SYSCALL_DEF(lgetxattr, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(2))
SYSCALL_DEF(fgetxattr, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(2))
SYSCALL_DEF(listxattr, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(llistxattr, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(flistxattr, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(removexattr, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_STRING)
SYSCALL_DEF(lremovexattr, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_STRING)
SYSCALL_DEF(fremovexattr, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_STRING)
SYSCALL_DEF(tkill, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(time, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(futex, SYSCALL_CAN_BE_FROM_ANYWHERE | SYSCALL_IS_RESTARTABLE, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(sched_setaffinity, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_SIZE_OF(2), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(sched_getaffinity, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_SIZE_OF(2), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(set_thread_area, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(io_setup, 0, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(io_destroy, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(io_getevents, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_COUNT_OF(3), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(io_submit, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_COUNT_OF(2), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(io_cancel, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(get_thread_area, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(lookup_dcookie, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(epoll_create, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(epoll_ctl_old, 0)
SYSCALL_DEF(epoll_wait_old, 0)
SYSCALL_DEF(remap_file_pages, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0), SYSCALL_ARG_IS_PROT, SYSCALL_ARG_IS_OFFSET, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(getdents64, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1))
SYSCALL_DEF(set_tid_address, 0, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(restart_syscall, SYSCALL_CAN_BE_FROM_ANYWHERE)
SYSCALL_DEF(semtimedop, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(fadvise64, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_OFFSET, SYSCALL_ARG_IS_OFFSET, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(timer_create, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(timer_settime, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(timer_gettime, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(timer_getoverrun, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(timer_delete, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(clock_settime, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(clock_gettime, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(clock_getres, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(clock_nanosleep, SYSCALL_IS_RESTARTABLE, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(exit_group, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(epoll_wait, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(epoll_ctl, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(tgkill, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(utimes, 0, SYSCALL_ARG_IS_PATH | SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(vserver, 0, 0)
SYSCALL_DEF(mbind, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0), SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(3), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(set_mempolicy, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(2))
SYSCALL_DEF(get_mempolicy, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(mq_open, 0, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_MODEFLAGS_OF(1), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(mq_unlink, 0, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(mq_timedsend, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1), SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(mq_timedreceive, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(mq_notify, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(mq_getsetattr, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(kexec_load, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_COUNT_OF(2), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(waitid, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(add_key, 0, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(2), SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(request_key, 0, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(keyctl, 0, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(ioprio_set, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(ioprio_get, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(inotify_init, 0)
SYSCALL_DEF(inotify_add_watch, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(inotify_rm_watch, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(migrate_pages, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_COUNT_OF(2), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(openat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_MODEFLAGS_OF(2))
SYSCALL_DEF(mkdirat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(mknodat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(fchownat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(futimesat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(newfstatat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(unlinkat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(renameat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(linkat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(symlinkat, 0, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(readlinkat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(2))
SYSCALL_DEF(fchmodat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(faccessat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(pselect6, 0, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(ppoll, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(0), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(3))
SYSCALL_DEF(unshare, 0, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(set_robust_list, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(0))
SYSCALL_DEF(get_robust_list, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(splice, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(tee, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(sync_file_range, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_OFFSET, SYSCALL_ARG_IS_OFFSET, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(vmsplice, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(move_pages, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_COUNT_OF(2), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(utimensat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(epoll_pwait, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(signalfd, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(timerfd_create, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(eventfd, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(fallocate, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_OFFSET, SYSCALL_ARG_IS_OFFSET)
SYSCALL_DEF(timerfd_settime, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(timerfd_gettime, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(accept4, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(signalfd4, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(eventfd2, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(epoll_create1, 0, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(dup3, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(pipe2, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(inotify_init1, 0, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(preadv, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_OFFSET, SYSCALL_ARG_IS_OFFSET)
SYSCALL_DEF(pwritev, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_OFFSET, SYSCALL_ARG_IS_OFFSET)
SYSCALL_DEF(rt_tgsigqueueinfo, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(perf_event_open, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(recvmmsg, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(fanotify_init, 0, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(fanotify_mark, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH)
SYSCALL_DEF(prlimit64, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(name_to_handle_at, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(open_by_handle_at, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(clock_adjtime, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(syncfs, 0, SYSCALL_ARG_IS_FD)
SYSCALL_DEF(sendmmsg, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(setns, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(getcpu, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(process_vm_readv, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(3), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(process_vm_writev, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(3), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(kcmp, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(finit_module, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(sched_setattr, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(sched_getattr, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(renameat2, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(seccomp, 0, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(getrandom, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(memfd_create, 0, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(kexec_file_load, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_SIZE_OF(3), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(bpf, 0, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1))
SYSCALL_DEF(execveat, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(userfaultfd, 0, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(membarrier, 0, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(mlock2, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(copy_file_range, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(preadv2, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1), SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(pwritev2, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(1), SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(pkey_mprotect, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0), SYSCALL_ARG_IS_PROT, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(pkey_alloc, 0, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(pkey_free, 0, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(statx, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(io_getevents, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_COUNT_OF(3), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(rseq, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(0), SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF_EMPTY()
SYSCALL_DEF(pidfd_send_signal, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(io_uring_setup, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(io_uring_enter, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(4))
SYSCALL_DEF(io_uring_register, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(2))
SYSCALL_DEF(open_tree, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(move_mount, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(fsopen, 0, SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(fsconfig, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_STRING, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(fsmount, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(fspick, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(pidfd_open, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(clone3, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0))
SYSCALL_DEF(close_range, 0, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(openat2, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(2))
SYSCALL_DEF(pidfd_getfd, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(faccessat2, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(process_madvise, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(epoll_pwait2, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(1), SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(mount_setattr, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PATH, SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(3))
SYSCALL_DEF(quotactl_fd, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_PRESERVED(SYSCALL_ARG_IS_INTEGER), SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS)
SYSCALL_DEF(landlock_create_ruleset, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0), SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(landlock_add_rule, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(landlock_restrict_self, 0, SYSCALL_ARG_IS_FD, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(memfd_secret, 0, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(process_mrelease, 0, SYSCALL_ARG_IS_PID, SYSCALL_ARG_IS_FLAGS)
SYSCALL_DEF(futex_waitv, SYSCALL_IS_RESTARTABLE, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_COUNT_OF(0), SYSCALL_ARG_IS_FLAGS, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_INTEGER)
SYSCALL_DEF(set_mempolicy_home_node, 0, SYSCALL_ARG_IS_ADDRESS, SYSCALL_ARG_IS_SIZE_OF(0), SYSCALL_ARG_IS_INTEGER, SYSCALL_ARG_IS_FLAGS)
