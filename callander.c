...
static const char *open_flags[64] = {
	// DESCRIBE_FLAG(O_RDONLY),
	DESCRIBE_FLAG(O_WRONLY),
	DESCRIBE_FLAG(O_RDWR),
...
struct enum_option access_modes[64] = {
	DESCRIBE_ENUM(F_OK),
};
...
static const char *at_flags[64] = {
	// DESCRIBE_FLAG(AT_EACCESS), // conflicts with AT_REMOVEDIR
...
struct enum_option socket_domains[] = {
...
struct enum_option socket_types[] = {
...
struct enum_option socket_levels[] = {
...
struct enum_option socket_options[] = {
...
static char *copy_enum_flags_description(const struct loader_context *context, struct register_state state, const struct enum_option *options, size_t sizeof_options, const char *flags[64])
{
	...
	if (length == 0 || remaining != 0) {
		...
		char num_buf[64];
		if (suffix == NULL && remaining != 0) {
			suffix = num_buf;
			suffix_len = fs_utoah(remaining, num_buf);
			length += suffix_len + 1;
		}
	}
	// allocate buffer
	...
	if (suffix_len != 0) {
		fs_memcpy(next, suffix, suffix_len + 1);
	} else {
		next--;
		*next = '\0';
	}
...
__attribute__((unused))
__attribute__((nonnull(1, 2, 4)))
static char *copy_call_description(const struct loader_context *context, const char *name, struct registers registers, const int *register_indexes, struct syscall_info info, bool include_symbol)
{
		...
		if (include_symbol) {
			switch (info.arguments[i] & SYSCALL_ARG_TYPE_MASK) {
				case SYSCALL_ARG_IS_PROT:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], prots, sizeof(prots), prot_flags);
					break;
				case SYSCALL_ARG_IS_MAP_FLAGS:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], maps, sizeof(maps), map_flags);
					break;
				case SYSCALL_ARG_IS_REMAP_FLAGS:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], NULL, 0, remap_flags);
					break;
				case SYSCALL_ARG_IS_OPEN_FLAGS:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], opens, sizeof(opens), open_flags);
					break;
				case SYSCALL_ARG_IS_SIGNUM:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], signums, sizeof(signums), NULL);
					break;
				case SYSCALL_ARG_IS_IOCTL:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], ioctls, sizeof(ioctls), NULL);
					break;
				case SYSCALL_ARG_IS_SIGHOW:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], sighows, sizeof(sighows), NULL);
					break;
				case SYSCALL_ARG_IS_MADVISE:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], madvises, sizeof(madvises), NULL);
					break;
				case SYSCALL_ARG_IS_FCNTL:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], fcntls, sizeof(fcntls), NULL);
					break;
				case SYSCALL_ARG_IS_RLIMIT:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], rlimits, sizeof(rlimits), NULL);
					break;
				case SYSCALL_ARG_IS_SOCKET_DOMAIN:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], socket_domains, sizeof(socket_domains), NULL);
					break;
				case SYSCALL_ARG_IS_SOCKET_TYPE:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], socket_types, sizeof(socket_types), NULL);
					break;
				case SYSCALL_ARG_IS_CLOCK_ID:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], clock_ids, sizeof(clock_ids), NULL);
					break;
				case SYSCALL_ARG_IS_SOCKET_LEVEL:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], socket_levels, sizeof(socket_levels), NULL);
					break;
				case SYSCALL_ARG_IS_SOCKET_OPTION:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], socket_options, sizeof(socket_options), NULL);
					break;
				case SYSCALL_ARG_IS_ACCESS_MODE:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], access_modes, sizeof(access_modes), access_mode_flags);
					break;
				case SYSCALL_ARG_IS_AT_FLAGS:
					args[i] = copy_enum_flags_description(context, registers.registers[reg], NULL, 0, at_flags);
					break;
