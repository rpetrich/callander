...
{
	if (nr < sizeof(syscall_list) / sizeof(syscall_list[0])) {
		return syscall_list[nr].valid_args;
	}
	return 6;
}
			...
			if (SHOULD_LOG) {
				if (register_is_partially_known(&regs->registers[i])) {
					if (register_index == REGISTER_RSP) {
						...
						ERROR("stack address was taken at", temp_str(copy_address_description(loader, regs->stack_address_taken)));
					}
				}
			}
...
static char *copy_address_list_description(const struct loader_context *context, const struct address_list_entry *head);
	...
	if (SHOULD_LOG) {
		...
		for (uint32_t i = 0; i < data->count; i++) {
			if (i == token->entry_index) {
				ERROR("existing values (current index)", (intptr_t)i);
			} else {
...
#else
		ERROR("existing values (current index)", (intptr_t)token->entry_offset);
...
static inline struct x86_ins_prefixes decode_x86_ins_prefixes(const uint8_t **ins) {
	...
	if (UNLIKELY(**ins == 0x3e)) {
		// notrack prefix for CET. not used by callander
	...
	for (size_t i = 0; i < symbols->symbol_count; i++) {
		...
		if (ELF64_ST_BIND(symbol->st_info) == STB_GLOBAL && ELF64_ST_TYPE(symbol->st_info) == STT_FUNC) {
			...
			if (protection_for_address_in_binary(binary, (uintptr_t)ins, NULL) & PROT_EXEC) {
				...
				struct analysis_frame new_caller = { .current = { .address = ins, .description = symbol_name(symbols, symbol), .next = &caller->current }, .current_state = empty_registers, .entry = binary->info.base, .entry_state = &empty_registers, .token = { 0 } };
				analyze_instructions(analysis, EFFECT_PROCESSED | EFFECT_AFTER_STARTUP, &empty_registers, ins, &new_caller, true);
			} else {
...
static void handle_dlopen(struct program_state *analysis, struct registers *state, __attribute__((unused)) const uint8_t *ins, struct analysis_frame *caller, __attribute__((unused)) struct effect_token *token, __attribute__((unused)) void *data)
{
	...
	if (!register_is_exactly_known(first_arg)) {
		...
		ERROR("dlopen call stack", temp_str(copy_address_list_description(&analysis->loader, &caller->current)));
		ERROR_FLUSH();
		abort();
	}
...
static void update_known_symbols(struct program_state *analysis, struct loaded_binary *new_binary)
{
	...
	if (new_binary->special_binary_flags & (BINARY_IS_LIBC | BINARY_IS_INTERPRETER)) {
		...
		if (analysis->ld_profile == NULL) {
			if (!update_known_function(analysis, new_binary, "_dl_start_profile", NORMAL_SYMBOL | LINKER_SYMBOL | DEBUG_SYMBOL_FORCING_LOAD, EFFECT_RETURNS | EFFECT_PROCESSED | EFFECT_AFTER_STARTUP | EFFECT_ENTRY_POINT)) {
				LOG("missing _dl_start_profile in", new_binary->path);
			}
		}
...
{
	size_t count = 0;
	for (const struct address_list_entry *node = head; node != NULL; node = node->next) {
		count++;
	}
	if (count == 0) {
		char *empty = malloc(1);
	...
	const struct address_list_entry *node = head;
	size_t total_size = 0;
	for (size_t i = 0; i < count; i++) {
...
{
	return copy_call_description(context, name_for_syscall(nr), registers, syscall_argument_abi_register_indexes, argc_for_syscall(nr) & SYSCALL_ARGC_MASK, include_symbol);
}
...
#pragma GCC unroll 64
		for (int i = 0; i < REGISTER_COUNT; i++) {
			...
			new_relevant_registers |= (relevant_registers & ((register_mask)1 << i)) ? s : 0;
			new_preserved_registers |= (preserved_registers & ((register_mask)1 << i)) ? s : 0;
			new_preserved_and_kept_registers |= (preserved_and_kept_registers & ((register_mask)1 << i)) ? s : 0;
		}
...
#pragma GCC unroll 64
		...
		if (UNLIKELY(log)) {
			...
			ERROR("from ins at", temp_str(copy_address_description(loader, ancestor->current.address)));
		}
		...
		ancestor = (struct analysis_frame *)ancestor->current.next;
		if (ancestor == NULL) {
			if (UNLIKELY(log)) {
...
void record_syscall(struct program_state *analysis, uintptr_t nr, struct analysis_frame self, function_effects effects)
{
	...
	if (config & SYSCALL_CONFIG_DEBUG) {
		...
		ERROR("at", temp_str(copy_address_list_description(&analysis->loader, &self.current)));
	}
	// figure out which, if any, arguments to the function were used in the syscall
	register_mask relevant_registers = syscall_argument_abi_used_registers_for_argc[argc & SYSCALL_ARGC_MASK];
	// determine which registers to preserve
	register_mask preserved_registers = syscall_argument_abi_used_registers_for_argc[0];
	for (int i = 0; i < 6; i++) {
		if (argc & (SYSCALL_ARG_IS_PRESERVED_BASE << i)) {
...
static inline mod_rm_t read_modrm(const uint8_t *byte) {
	union {
		uint8_t byte;
		mod_rm_t modrm;
	} pun;
...
static inline int read_reg(mod_rm_t modrm, struct x86_ins_prefixes rex) {
	return modrm.reg + (rex.has_r << 3);
}

static inline int read_rm(mod_rm_t modrm, struct x86_ins_prefixes rex) {
	return modrm.rm + (rex.has_b << 3);
}

__attribute__((always_inline))
static inline bool modrm_is_direct(mod_rm_t modrm) {
	return modrm.mod == 3;
}
...
static inline sib_t read_sib(const uint8_t *byte) {
	union {
		uint8_t byte;
		sib_t sib;
	} pun;
...
static inline int read_base(sib_t sib, struct x86_ins_prefixes rex) {
	return sib.base + (rex.has_b << 3);
}

static inline int read_index(sib_t sib, struct x86_ins_prefixes rex) {
	return sib.index + (rex.has_x << 3);
}

static inline int read_opcode_register_index(uint8_t opcode_value, uint8_t opcode_start, struct x86_ins_prefixes rex)
{
	return opcode_value - opcode_start + ((int)rex.has_b << 3);
}

static inline struct register_state_and_source address_for_indirect(struct x86_ins_prefixes rex, mod_rm_t modrm, struct registers state, const uint8_t *data, const struct loader_context *loader, const uint8_t *ins, const uint8_t **out_remaining, bool *out_base_is_null) {
	...
	switch (rm) {
		...
		case REGISTER_R12: {
			// decode SIB
			sib_t sib = read_sib(data++);
			int base_reg = read_base(sib, rex);
			...
			int index_reg = read_index(sib, rex);
...
struct decoded_rm decode_rm(const uint8_t **ins_modrm, struct x86_ins_prefixes rex, uint8_t imm_size) {
	mod_rm_t modrm = read_modrm(*ins_modrm);
	*ins_modrm += sizeof(mod_rm_t);
	...
	if (rex.has_segment_override) {
		...
		switch ((result.rm = read_rm(modrm, rex))) {
			case REGISTER_RSP:
			case REGISTER_R12: {
				// decode SIB
				sib_t sib = read_sib(*ins_modrm);
				*ins_modrm += sizeof(sib_t);
				result.base = read_base(sib, rex);
				result.index = read_index(sib, rex);
...
{
	int reg = read_reg(read_modrm(ins_modrm), rex);
...
function_effects analyze_instructions(struct program_state *analysis, function_effects required_effects, const struct registers *entry_state, const uint8_t *ins, struct analysis_frame *caller, enum jump_table_status jump_status)
{
	...
	while (UNLIKELY(x86_jump_addresses_at_instruction(UNLIKELY(x86_is_endbr64_instruction(ins)) ? &ins[4] : ins, &jump_target) == X86_JUMPS_ALWAYS)) {
	...
	self.current.next = &caller->current;
	self.entry = ins;
#pragma GCC unroll 64
	for (int i = 0; i < REGISTER_COUNT; i++) {
		self.current_state.sources[i] = (register_mask)1 << i;
	...
	for (;;) {
		...
		switch (x86_jump_addresses_at_instruction(ins, &jump_target)) {
			...
			case X86_JUMPS_ALWAYS: {
				...
				if (jump_target == NULL) {
					...
					ERROR("found single jump to non-executable address at", temp_str(copy_address_list_description(&analysis->loader, &self.current)));
					...
					effects |= analyze_instructions(analysis, required_effects, &self.current_state, jump_target, &self, true) & ~(EFFECT_AFTER_STARTUP | EFFECT_PROCESSING);
		...
		if (*unprefixed == 0xff) {
			...
			if (modrm.reg == 2) { // TODO: do we need this?
				...
				if (modrm_is_direct(modrm)) {
					...
					vary_effects_by_registers(&analysis->search, &analysis->loader, &self, self.current_state.sources[reg], 0, 0, required_effects, SHOULD_LOG);
...
#if ABORT_AT_NON_EXECUTABLE_ADDRESS
						ERROR("found call* at", temp_str(copy_address_list_description(&analysis->loader, &self.current)));
					...
					if (!register_is_exactly_known(&self.current_state.registers[reg])) {
						switch (jump_status) {
							case DISALLOW_JUMPS_INTO_THE_ABYSS:
								ERROR("jmpq* to unknown address", temp_str(copy_address_description(&analysis->loader, self.current.address)));
								DIE("trace", temp_str(copy_address_list_description(&analysis->loader, &self.current)));
								break;
							case ALLOW_JUMPS_INTO_THE_ABYSS:
								LOG("jmpq* to unknown address", temp_str(copy_address_description(&analysis->loader, self.current.address)));
								dump_nonempty_registers(&analysis->loader, &self.current_state, ALL_REGISTERS);
								break;
					...
					if ((protection_for_address(&analysis->loader, (const void *)address.state.value, NULL, NULL) & PROT_READ) == 0) {
						switch (jump_status) {
							case DISALLOW_JUMPS_INTO_THE_ABYSS:
								...
								DIE("at", temp_str(copy_address_list_description(&analysis->loader, &self.current)));
								break;
							case ALLOW_JUMPS_INTO_THE_ABYSS:
								...
								self.current.description = NULL;
								LOG("at", temp_str(copy_address_list_description(&analysis->loader, &self.current)));
								break;
							case DISALLOW_AND_PROMPT_FOR_DEBUG_SYMBOLS: {
								print_debug_symbol_requirement(binary_for_address(&analysis->loader, ins));
								ERROR_FLUSH();
				...
				if (new_ins == NULL) {
					...
					effects |= EFFECT_EXITS | EFFECT_RETURNS;
#if ABORT_AT_NON_EXECUTABLE_ADDRESS
					ERROR("found jmp* to non-executable address at", temp_str(copy_address_list_description(&analysis->loader, &self.current)));
					...
					effects |= analyze_instructions(analysis, required_effects & ~EFFECT_ENTRY_POINT, &self.current_state, new_ins, caller, true) & ~(EFFECT_AFTER_STARTUP | EFFECT_PROCESSING);
					LOG("completing from jmpq*", temp_str(copy_address_description(&analysis->loader, self.entry)));
				}
				goto update_and_return;
			}
		...
		switch (*unprefixed) {
				...
				switch (unprefixed[1]) {
						...
						if (register_is_exactly_known(&self.current_state.registers[REGISTER_RAX])) {
							...
							switch (value) {
								...
								case __NR_exit_group:
									// exit and exitgroup always exit the current thread
									effects |= EFFECT_EXITS;
							...
							if (SHOULD_LOG) {
								...
								for (const struct analysis_frame *ancestor = &self;;) {
									ERROR("from call site", temp_str(copy_address_description(&analysis->loader, ancestor->current.address)));
									register_mask new_relevant_registers = 0;
									for (int i = 0; i < REGISTER_COUNT; i++) {
										if (relevant_registers & ((register_mask)1 << i)) {
							...
							ERROR("full call stack", temp_str(copy_address_list_description(&analysis->loader, &self.current)));
							dump_nonempty_registers(&analysis->loader, &self.current_state, ALL_REGISTERS);
							clear_register(&self.current_state.registers[REGISTER_RAX]);
							self.current_state.sources[REGISTER_RAX] = 0;
							clear_match(&analysis->loader, &self.current_state, REGISTER_RAX, ins);
					...
					case 0x11: { // movups xmm/m, xmm
						...
						if (rm >= REGISTER_MEM) {
							...
							mod_rm_t modrm = read_modrm(&unprefixed[2]);
							if (read_reg(modrm, rex) == REGISTER_R15 && binary_has_flags(binary_for_address(&analysis->loader, ins), BINARY_IS_GOLANG)) {
					...
					case 0x38:
						switch (unprefixed[2]) {
							...
							case 0xf0: { // movbe r, r/m or crc32 r, r/m8
								mod_rm_t modrm = read_modrm(&unprefixed[3]);
								int reg = read_reg(modrm, rex);
								clear_register(&self.current_state.registers[reg]);
								self.current_state.sources[reg] = 0;
								clear_match(&analysis->loader, &self.current_state, reg, ins);
					...
					case 0x57: { // xorps xmm1, xmm2/m128
						...
						if (modrm_is_direct(modrm) && reg == read_rm(modrm, rex)) {
							...
							struct x86_ins_prefixes lookahead_rex = decode_x86_ins_prefixes(&unprefixed);
							if (lookahead[0] == 0x0f && lookahead[1] == 0x11) { // movups xmm2/m128, xmm1
								lookahead += 2;
								mod_rm_t lookahead_modrm = read_modrm(lookahead);
								if (reg == read_reg(lookahead_modrm, lookahead_rex)) {
					...
					case 0xb7: { // movzx r, r/m16
						LOG("found movzx");
						int dest = read_reg(read_modrm(&unprefixed[2]), rex);
						LOG("to", name_for_register(dest));
						const uint8_t *remaining = &unprefixed[2];
							...
							if (self.current_state.mem_rm.rm == REGISTER_STACK_0 && self.current_state.mem_rm.index != REGISTER_RSP) {
								...
								if (protection_for_address(&analysis->loader, (const void *)(base_addr + value * sizeof(uint32_t)), NULL, NULL) & PROT_READ) {
									if (max - value > MAX_LOOKUP_TABLE_SIZE) {
										...
										self.current.description = "lookup table";
					...
					case 0xce:
					case 0xcf: {
						int reg = read_opcode_register_index(unprefixed[1], 0xc8, rex);
						clear_register(&self.current_state.registers[reg]);
						if (!rex.has_w) {
			...
			case 0x56:
			case 0x57: {
				int reg = read_opcode_register_index(*unprefixed, 0x50, rex);
			...
			case 0x5e:
			case 0x5f: {
				int reg = read_opcode_register_index(*unprefixed, 0x58, rex);
			...
			case 0x63: {
				...
				if (modrm.mod != 0x3 && (modrm.rm == REGISTER_R12 || modrm.rm == REGISTER_RSP)) {
					// read SIB
					sib_t sib = read_sib(&unprefixed[2]);
					int base = read_base(sib, rex);
					int index = read_index(sib, rex);
			...
			case 0x8d: { // lea r, r/m (only indirect!)
				...
				if (modrm_is_direct(modrm)) {
					self.current.description = NULL;
					LOG("lea with direct addressing mode at", temp_str(copy_address_list_description(&analysis->loader, &self.current)));
					break;
				}
				int reg = read_reg(modrm, rex);
				...
				if (new_value.source & ((register_mask)1 << REGISTER_RSP)) {
					// if (reg == REGISTER_RBP) {
					// 	LOG("ignoring address of stack (since it's to rbp)", temp_str(copy_address_description(&analysis->loader, self.current.address)));
					// } else
					record_stack_address_taken(&analysis->loader, self.current.address, &self.current_state);
				}
							...
							if (should_skip_lea) {
								...
								if (required_effects & EFFECT_ENTRY_POINT) {
									if (reg == sysv_argument_abi_register_indexes[0]) {
										...
										analyze_instructions(analysis, (required_effects & ~EFFECT_ENTRY_POINT), &empty_registers, address, &self, true);
									}
								} else {
									LOG("rip-relative lea is to executable address, assuming it could be called after startup");
									struct registers empty = empty_registers;
									queue_instruction(&analysis->search.queue, address, (required_effects & ~EFFECT_ENTRY_POINT) | EFFECT_AFTER_STARTUP, empty, self.current.address, "lea");
									//analyze_instructions(analysis, (required_effects & ~EFFECT_ENTRY_POINT) | EFFECT_AFTER_STARTUP, &empty_registers, address, &self, true);
								}
							}
			...
			case 0x96:
			case 0x97: {
				int reg = read_opcode_register_index(*unprefixed, 0x90, rex);
				struct register_state dest = self.current_state.registers[reg];
			...
			case 0xb6:
			case 0xb7: {
				int reg = read_opcode_register_index(*unprefixed, 0xb0, rex);
				LOG("mov r8 to", name_for_register(reg));
				struct register_state dest;
			...
			case 0xbe:
			case 0xbf: {
				int reg = read_opcode_register_index(*unprefixed, 0xb8, rex);
				LOG("mov r to", name_for_register(reg));
				struct register_state dest;
				...
				if (protection_for_address(&analysis->loader, address, NULL, NULL) & PROT_EXEC) {
					...
					queue_instruction(&analysis->search.queue, address, (required_effects & ~EFFECT_ENTRY_POINT) | EFFECT_AFTER_STARTUP, empty_registers, self.current.address, "mov");
					// self.current.description = "mov";
					// analyze_instructions(analysis, (required_effects & ~EFFECT_ENTRY_POINT) | EFFECT_AFTER_STARTUP, &empty_registers, (const uint8_t *)address, &self, true);
				}
				break;
			}
					...
					if (protection_for_address(&analysis->loader, (const void *)state.value, NULL, NULL) & PROT_EXEC) {
						...
						if (required_effects & EFFECT_ENTRY_POINT) {
							if (rm == sysv_argument_abi_register_indexes[0]) {
								...
								analyze_instructions(analysis, (required_effects & ~EFFECT_ENTRY_POINT) | EFFECT_AFTER_STARTUP, &empty_registers, (const uint8_t *)state.value, &self, true);
								...
								analyze_instructions(analysis, (required_effects & ~EFFECT_ENTRY_POINT), &empty_registers, (const uint8_t *)state.value, &self, true);
							}
						} else {
							LOG("mov is to executable address, assuming it could be called after startup");
							queue_instruction(&analysis->search.queue, (const uint8_t *)state.value, (required_effects & ~EFFECT_ENTRY_POINT) | EFFECT_AFTER_STARTUP, empty_registers, self.current.address, "mov");
							// analyze_instructions(analysis, (required_effects & ~EFFECT_ENTRY_POINT) | EFFECT_AFTER_STARTUP, &empty_registers, (const uint8_t *)state.value, &self, true);
						}
					} else {
...
static void analyze_golang_init_task(struct program_state *analysis, function_effects effects, const struct golang_init_task *task)
{
	...
	*entry |= effects & ~EFFECT_PROCESSING;
	...
	for (uintptr_t i = 0; i < nfns; i++) {
		...
		struct analysis_frame new_caller = { .current = { .address = &task->data[ndeps+i], .description = "golang task init", .next = NULL }, .current_state = registers, .entry = (const void *)task, .entry_state = &registers, .token = { 0 } };
		analyze_instructions(analysis, EFFECT_PROCESSED | effects, &registers, task->data[ndeps+i], &new_caller, true);
	}
}
			...
			case DT_NEEDED:
				if (new_binary->has_symbols) {
					...
					if (additional_binary) {
						int result = finish_loading_binary(analysis, additional_binary, effects, skip_analysis);
						if (result != 0) {
				...
				if (should_search) {
					...
					for (int i = 0; i < size; i++) {
						...
						if (protection_for_address_in_binary(new_binary, data, NULL) & PROT_EXEC) {
							...
							LOG("value of address is, assuming callable", data);
							if (find_skipped_symbol_for_address(&analysis->loader, new_binary, &section_data[i]) == NULL) {
								struct analysis_frame new_caller = { .current = { .address = &section_data[i], .description = name, .next = NULL }, .current_state = empty_registers, .entry = (void *)&section_data[i], .entry_state = &empty_registers, .token = { 0 } };
								analyze_instructions(analysis, EFFECT_PROCESSED | EFFECT_AFTER_STARTUP | effects, &registers, (const uint8_t *)data, &new_caller, true);
							}
						}
					}
				} else {
...
void sort_and_coalesce_syscalls(struct recorded_syscalls *syscalls, struct loader_context *loader)
{
	...
	for (int i = 0; i < count - 1; ) {
		...
		int argc = argc_for_syscall(earlier->nr);
		register_mask relevant_registers = syscall_argument_abi_used_registers_for_argc[argc & SYSCALL_ARGC_MASK];
...
char *copy_used_syscalls(const struct loader_context *context, const struct recorded_syscalls *syscalls, bool log_arguments, bool log_caller, bool include_symbol)
{
	...
	for (int i = 0; i < count; i++) {
		...
		if (log_arguments || i == 0 || list[i-1].nr != nr) {
			...
			if (log_caller && (argc_for_syscall(nr) & SYSCALL_CAN_BE_FROM_ANYWHERE) == 0) {
				log_len += 3; // " @ ";
...
char *copy_used_binaries(const struct loader_context *loader)
{
	...
	for (struct loaded_binary *binary = loader->last; binary != NULL; binary = binary->previous) {
		...
		size_t len = fs_strlen(binary->path);
...
struct sock_fprog generate_seccomp_program(struct loader_context *loader, struct recorded_syscalls *syscalls, enum seccomp_validation_mode validation_mode, uint32_t syscall_range_low, uint32_t syscall_range_high) {
	...
	for (int i = 0; i < count;) {
		...
		int argc = argc_for_syscall(nr);
		if (validation_mode >= VALIDATE_SYSCALL_AND_CALL_SITE && list[i].ins != NULL && (argc & SYSCALL_CAN_BE_FROM_ANYWHERE) == 0) {
				...
				do {
					...
					if (validation_mode >= VALIDATE_ALL) {
						...
						do {
							...
							for (int j = 0; j < (argc & SYSCALL_ARGC_MASK); j++) {
							...
							for (int j = 0; j < (argc & SYSCALL_ARGC_MASK); j++) {
								size_t hi_pos = arg_pos[j].compare_hi;
								if (hi_pos != 0) {
...
void finish_analysis(struct program_state *analysis)
{
	...
	while (dequeue_instruction(&analysis->search.queue, &ins)) {
		...
		struct analysis_frame queued_caller = { .current = { .address = ins.caller, .description = ins.description, .next = NULL }, .current_state = empty_registers, .entry = ins.caller, .entry_state = &empty_registers, .token = { 0 } };
		analyze_instructions(analysis, ins.effects, &ins.registers, ins.ins, &queued_caller, true);
	}
