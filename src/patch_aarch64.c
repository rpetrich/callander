...
static enum patch_status patch_common(struct thread_storage *thread, ins_ptr instruction, void *handler, bool skip, int self_fd);
...
void patch_body(struct thread_storage *thread, struct patch_body_args *args)
{
	...
	args->patched = patch_common(thread, instruction, &receive_trampoline, true, args->self_fd);
}
...
static enum patch_status patch_common(struct thread_storage *thread, ins_ptr instruction, void *handler, bool skip, int self_fd)
{
	...
	if (current_region && trampoline_region_has_space((uint8_t *)current_region, LARGEST_TRAMPOLINE) && is_valid_pc_relative_offset((intptr_t)current_region - ((uintptr_t)instruction + sizeof(uint32_t)))) {
	...
	size_t template_size = (uintptr_t)&trampoline_call_handler_end - (uintptr_t)&trampoline_call_handler_start - sizeof(uint64_t);
