...
#define AARCH64_FAKE_DEBUGGER_FRAMES

__asm__(
	".pushsection .rodata\n"
	FS_HIDDEN_FUNCTION_ASM(trampoline_call_handler_start) "\n"
	FS_HIDDEN_FUNCTION_ASM(trampoline_call_handler_data) "\n"
		"0:\n"
#ifdef AARCH64_FAKE_DEBUGGER_FRAMES
		".quad 0\n"
#endif
		"1:\n"
		".quad 0\n"
	FS_HIDDEN_FUNCTION_ASM(trampoline_call_handler_entry) "\n"
		"	stp x9, x10, [sp, #-0x90]!\n"
		"	stp x11, x12, [sp, #-0x10]!\n"
		"	stp x13, x14, [sp, #-0x10]!\n"
		"	stp x15, xzr, [sp, #-0x10]!\n"
		// save frame for patched function's caller
		"   stp fp, lr, [sp, #-0x10]!\n"
#ifdef AARCH64_FAKE_DEBUGGER_FRAMES
		// save frame for patched function
		"   mov x9, sp\n"
		"   ldr x10, 0b\n"
		"   stp x9, x10, [sp, #-0x10]!\n"
		// save frame for self
		...
		"	stp x2, x3, [sp, #-0x10]!\n"
		"	stp x0, x1, [sp, #-0x10]!\n"
		// fill args and call trampoline_body
		"	mov x0, sp\n"
		"	blr x13\n"
		// restore args and flats
		"2:\n"
		"	ldp x0, x1, [sp], #0x10\n"
		"	ldp x2, x3, [sp], #0x10\n"
		"	ldp x4, x5, [sp], #0x10\n"
...
__asm__(
	...
	FS_HIDDEN_FUNCTION_ASM(breakpoint_call_handler_data) "\n"
#ifdef AARCH64_FAKE_DEBUGGER_FRAMES
		".quad 0\n"
#endif
			"1:\n"
		".quad 0\n"
	FS_HIDDEN_FUNCTION_ASM(breakpoint_call_handler_entry) "\n"
		"	stp x30, xzr, [sp, #-0x90]!\n"
		"	stp x28, x29, [sp, #-0x10]!\n"
		"	stp x26, x27, [sp, #-0x10]!\n"
		"	stp x24, x25, [sp, #-0x10]!\n"
...
static enum patch_status patch_common(struct thread_storage *thread, ins_ptr instruction, struct patch_template template, void *handler, bool skip, int self_fd)
{
	...
	int mapping_result = patch_cached_mapping_for_address(instruction, &target_mapping);
	if (mapping_result <= 0) {
		if (mapping_result < 0) {
	...
	size_t template_size = (uintptr_t)template.end - (uintptr_t)template.start;
	fs_memcpy((void *)stub_address, (const void *)template.start, template_size);
	// copy original instruction
	uint32_t *trampoline = (uint32_t *)(stub_address + template_size);
	if (!skip) {
...
#endif
		void *handler;
	} *data = (void *)(stub_address + (uintptr_t)template.data - (uintptr_t)template.start);
#ifdef AARCH64_FAKE_DEBUGGER_FRAMES
	data->lr = instruction + sizeof(uint32_t);
#endif
...
#ifdef __x86_64__
	// the mprotect above is enough make the trampoline visible
	if (membarrier_is_supported) {
		fs_membarrier(MEMBARRIER_CMD_PRIVATE_EXPEDITED, 0);
	}
#endif
#endif
	intptr_t offset = stub_address + (uintptr_t)template.entry - (uintptr_t)template.start - (uintptr_t)instruction;
	patch_write_pc_relative_jump(instruction, offset);
	clear_icache((uintptr_t)instruction, (uintptr_t)&instruction[1]);
	if (mapping_result == 1) {
		int result = fs_mprotect((void *)start_page, PAGE_SIZE, target_mapping.prot);
		if (result < 0) {
