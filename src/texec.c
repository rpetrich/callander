...
__attribute__((warn_unused_result)) intptr_t remote_mmap(intptr_t addr, size_t length, int prot, int flags, int fd, off_t offset)
{
	...
	if (result < 0) {
		DIE("failed writing remote mmap contents: ", as_errno(result));
	}
...
int remote_load_binary(int fd, struct binary_info *out_info)
{
	...
	if (fs_is_map_failed((void *)mapped_address)) {
		ERROR("could not map binary: ", as_errno((intptr_t)mapped_address));
		return -ENOEXEC;
	}
	...
	for (int i = 0; i < header.e_phnum; i++) {
		...
		if (ph->p_memsz > ph->p_filesz) {
			...
			fs_memset(zeros, '\0', zero_count);
			intptr_t result = proxy_poke(brk, zero_count, &zeros);
			if (result < 0) {
				DIE("failed to write zeros: ", as_errno(result));
			}
...
static void transfer_fd_table(uintptr_t fd_table_addr)
{
	...
	for (int i = 0; i < MAX_TABLE_SIZE; i++) {
		...
		if (value != 0) {
			...
			if (result < 0) {
				DIE("failed writing fd table: ", as_errno(result));
			}
		}
	}
	...
	if (local_table[CWD_FD] & HAS_LOCAL_FD) {
		...
		if (local_fd < 0) {
			DIE("failed opening current working directory: ", as_errno(local_fd));
		}
		int value = (local_fd << USED_BITS) | HAS_REMOTE_FD;
		intptr_t result = proxy_poke(fd_table_addr + sizeof(int) * CWD_FD, sizeof(int), &value);
		if (result < 0) {
			DIE("failed writing current working directory: ", as_errno(result));
		}
	}
}
...
static int process_syscalls_until_exit(struct remote_exec_state *remote, struct thandler_info *thandler)
{
	...
	if (result < 0) {
		ERROR("failed target proxy state: ", as_errno(result));
		return -ENOEXEC;
	}
	...
	if (remote->comm != NULL) {
		...
		if (result < 0) {
			ERROR("failed to set comm: ", as_errno(result));
			return result;
		}
	}
#endif
	...
	if (clone_result < 0) {
		ERROR("failed to clone: ", as_errno(clone_result));
		return clone_result;
	}
	if (debug) {
	...
	do {
		...
		if (result < 0) {
			DIE("failed to wait for thread to exit: ", as_errno(result));
		}
...
static void process_syscalls(char buf[512 * 1024], struct process_syscalls_data *data)
{
	...
	for (;;) {
		...
		if (UNLIKELY(result_id < 0)) {
			...
			DIE("proxy_read_stream_message_start returned unexpected error: ", as_errno(result_id));
			return;
		}
		switch (message.template.nr) {
				...
				if (addr >= heap && addr + size < &heap[sizeof(heap)]) {
					...
					while (size != bytes_read) {
						...
						if (result <= 0) {
							...
							DIE("Failed to read from socket: ", as_errno(result));
						}
						bytes_read += result;
					}
				} else {
			...
			case __NR_clone | PROXY_NO_RESPONSE: {
				...
				if (fs_is_map_failed(stack)) {
					ERROR("unable to map a worker stack: ", as_errno((intptr_t)stack));
					ERROR_FLUSH();
					break;
				}
				...
				if (result < 0) {
					...
					ERROR("unable to start a worker thread: ", as_errno(result));
					ERROR_FLUSH();
					break;
				}
				proxy_read_stream_message_finish(stream_id);
...
__attribute__((noinline, visibility("hidden"))) int main(__attribute__((unused)) int argc, char *argv[], char *envp[])
{
	...
	if (result < 0) {
		DIE("failed to load thandler: ", as_errno(result));
	}
